...
def crawling(link):
	...
	for li in link:  
		...
		driver.execute_script(f"window.scrollBy(0,{height+1000})")
		time.sleep(3)
		...
		try:
			...
			for sp in range(len(special_function)):
				...
				special.append([sp_img[sp].get_attribute('src'),txt[0], txt[-1]])
				print(special)
		except:
			print(' ')
		...
		# time.sleep(1)
		page4 = pd.DataFrame(columns=['Product name','Table'])
		try:
		...
		page1.append([li,breadcrumb,Category,Product_name,pricing,avail_size,sense_size,img_li,title_description,general_description,item_description,special,rating,count_rating,positive_pct,fit,length,quality,comfort,s])
		...
		df3.to_csv('review.csv',mode='a',index=False,header = False)
